...
#define DEFAULT_UPPER_BOUND 100.0
...
static void parse_cmd_line(int argc, char *argv[], struct s_settings *p_settings)
{
        ...
        while (i < argc)
        {
                ...
                else if (strcmp(argv[i], "--lower-bound") == 0)
                {
                        ...
                        double value = atof(argv[i]); //-----------------------------------------------------> changement pour compiler chat 
...
static void OpenMP_compute_histogram(const ELEMENT_TYPE *array, int *histogram, struct s_settings *p_settings)
{
...

        {
                ...
                int j;
                #pragma omp parallel for num_threads(32)              // omp for 
...
{
        OpenMP_compute_histogram(array, run_histogram, p_settings);
