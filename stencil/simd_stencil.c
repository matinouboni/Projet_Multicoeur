...
static void simd_stencil_func(ELEMENT_TYPE *p_mesh, struct s_settings *p_settings)
{
    ...
    const int width = p_settings->mesh_width;
    const int height = p_settings->mesh_height;

    ELEMENT_TYPE *p_temporary_mesh = (ELEMENT_TYPE *)_mm_malloc(width * height * sizeof(*p_mesh), 32);
    if (p_temporary_mesh == NULL)
    {
        PRINT_ERROR("memory allocation failed");
    }

    const __m256 coef = _mm256_set1_ps(0.25f);

    // Parcours des lignes du maillage
    for (int y = margin_y; y < height - margin_y; y++)
    {
        int x;
        // Traitement des colonnes par blocs de 8
        for (x = margin_x; x <= width - margin_x - 8; x += 8)
        {
            // Chargement des voisins
            __m256 top = _mm256_loadu_ps(&p_mesh[(y - 1) * width + x]);
            __m256 left = _mm256_loadu_ps(&p_mesh[y * width + x - 1]);
            __m256 right = _mm256_loadu_ps(&p_mesh[y * width + x + 1]);
            __m256 bottom = _mm256_loadu_ps(&p_mesh[(y + 1) * width + x]);
            ...
            __m256 value = _mm256_mul_ps(sum, coef);

            // Stockage du résultat
            _mm256_storeu_ps(&p_temporary_mesh[y * width + x], value);
        }
    ...
    // Copie du maillage temporaire vers le maillage principal
    for (int y = margin_y; y < height - margin_y; y++)
    {
        memcpy(&p_mesh[y * width + margin_x],
               &p_temporary_mesh[y * width + margin_x],
               (width - 2 * margin_x) * sizeof(ELEMENT_TYPE));
    }

    _mm_free(p_temporary_mesh);
}
...
static void run(ELEMENT_TYPE *p_mesh, struct s_settings *p_settings)
{
        ...
        {
                simd_stencil_func(p_mesh, p_settings); // Utilisation de la version SIMD
