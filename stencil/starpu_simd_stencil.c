...
static void allocate_mesh(ELEMENT_TYPE **pp_mesh, struct s_settings *p_settings)
{
    ...
    size_t size = p_settings->mesh_width * p_settings->mesh_height * sizeof(ELEMENT_TYPE);
    int ret = posix_memalign((void **)pp_mesh, alignment, size);
    if (ret != 0)
    {
        PRINT_ERROR("memory allocation failed");
    }
...
static void naive_stencil_func(ELEMENT_TYPE *p_mesh, struct s_settings *p_settings)
{
    ...
    ELEMENT_TYPE *p_temporary_mesh;
    size_t alignment = 32; // For AVX
    size_t size = p_settings->mesh_width * p_settings->mesh_height * sizeof(ELEMENT_TYPE);
    int ret = posix_memalign((void **)&p_temporary_mesh, alignment, size);
    if (ret != 0)
    {
        PRINT_ERROR("memory allocation failed");
    }
...
static void stencil_cpu_func(void *buffers[], void *cl_arg) {
    ...
    for(int y = margin_y; y < height + margin_y; y++) {
        int x;
        // SIMD vectorization using unaligned loads/stores
        for(x = margin_x; x <= width + margin_x - 8; x += 8) { // Process 8 elements at a time
            __m256 value = _mm256_loadu_ps(&p_mesh[y * ld + x]); // Initial value

            // Load stencil coefficients
            __m256 coef_center = _mm256_set1_ps(stencil_coefs[4]); // -1.00
            __m256 coef_adjacent = _mm256_set1_ps(0.25f);

            // Center point
            __m256 center = _mm256_loadu_ps(&p_mesh[y * ld + x]);
            value = _mm256_fmadd_ps(center, coef_center, value); // value += center * (-1.0)

            // Apply adjacent coefficients
            // Top
            __m256 top = _mm256_loadu_ps(&p_mesh[(y - 1) * ld + x]);
            value = _mm256_fmadd_ps(top, coef_adjacent, value);

            // Bottom
            __m256 bottom = _mm256_loadu_ps(&p_mesh[(y + 1) * ld + x]);
            value = _mm256_fmadd_ps(bottom, coef_adjacent, value);

            // Left
            __m256 left = _mm256_loadu_ps(&p_mesh[y * ld + x - 1]);
            value = _mm256_fmadd_ps(left, coef_adjacent, value);

            // Right
            __m256 right = _mm256_loadu_ps(&p_mesh[y * ld + x + 1]);
            value = _mm256_fmadd_ps(right, coef_adjacent, value);

            // Store result
            _mm256_storeu_ps(&p_temporary_mesh[y * ld + x], value);
        }

        // Handle remaining elements
        for(; x < width + margin_x; x++) {
            ELEMENT_TYPE value = p_mesh[y * ld + x];
            value += p_mesh[y * ld + x] * stencil_coefs[4]; // Center point

            value += p_mesh[(y - 1) * ld + x] * stencil_coefs[1]; // Top
            value += p_mesh[(y + 1) * ld + x] * stencil_coefs[7]; // Bottom
            value += p_mesh[y * ld + x - 1] * stencil_coefs[3];   // Left
            value += p_mesh[y * ld + x + 1] * stencil_coefs[5];   // Right
